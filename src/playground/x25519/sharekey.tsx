...
import { GenerateX25519KeyPair, GetShareKeyResult, SharedKeyTest, GetAESBase64Key } from './key';

const X25519ShareKeyApp = () => {
  ...
  const clickGenerateKeyButton = async (exportType: string) => {
    ...
    let resultA = await GenerateX25519KeyPair(exportType);
    // console.log(resultA);

    setPrivatekeyA(resultA.PrivateKey);
    setPubkeyA(resultA.PublicKey);

    let resultB = await GenerateX25519KeyPair(exportType);
    // console.log(resultB);
  ...
  return (
    <div>
      <h2>Generate Two X25519 key pairs</h2>
